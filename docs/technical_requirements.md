...
* имя и фамилия
* пароль
* **возможно — телефон. Вход для подписчиков по email+пароль или телефон+смс
код? Требует обсуждения**
* **возможно — что-то для привязки телеграма. Вопрос требует проработки**


## Аутентификация блогера и подписчиков

Аутентификация блогера и подписчиков осуществляется по email и паролю.
Для подписчиков должна быть реализована возможность восстановления забытого
пароля. При запросе восстановления пароля необходимо на email подписчика
прислать ссылку на восстановление пароля. При переходе по этой ссылке
пользователь должен иметь возможность задать новый пароль на вход.

Для блогера возможности восстановления пароля в интерфейсе системы
предусматривать не нужно — с точки зрения безопасности. Однако на сервере
должна быть предусмотрена возможность смены пароля блогеру через отдельную
команду. Процесс смены пароля для блогера должен быть описан в документации
Системы.


## Функционал для блогера
...
1. Редактирование данных профиляъ
...
### Заведение и редактирование типов подписки

По каждому типу подписки задаются:

* название подписки
* описание подписки
* **возможно — обложка подписки для использования где-то в диайзне покупки
...
или всем. Можно выбрать несколько вариантов, например, бесплатным подписчам
...
что приведёт к останоке рекуррентных ежемесячных платежей и после
...
логотип Системы где-то на странице. Логотип надо разраьбоать в рамках
этого проекта.
